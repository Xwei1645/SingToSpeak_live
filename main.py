'''部分代码来自TyTalk（https://github.com/GalaxieT/TyTalk）、bilibili-danmu（https://github.com/jonssonyan/bilibili-danmu）
SingToSpeak_live
作者：Xwei
B站主页：https://space.bilibili.com/573734644
(直播用）B站主页：https://space.bilibili.com/691973660
直播间：http://live.bilibili.com/25537837
'''
#版本号
version = '0.9.0'
#下载链接
download_url = 'https://s2s.rth7.com/download'
...
Sletter2Bletter = str.maketrans("abcdefghijklmnopqrstuvwxyz",'ABCDEFGHIJKLMNOPQRSTUVWXYZ')
Bletter2hans = str.maketrans("ABCDEFGHIJKLMNOPQRSTUVWXYZ",'啊波呲的呃夫哥哈一鸡柯了么呢哦泼丘日思特乌微挖溪呀资')
...
class Talker:
    ...
    def load(self, ipt=''):  # text to phonemes
        """
        ...
        self.total_time = 0
        text = arabToChi.convert(ipt)
        fil = re.compile(
            r'[^0-9\u4e00-\u9fa5.， ,\-。；%《*》/•、&＆(—)（+）…：？!！“”·\n]+')  # https://www.jianshu.com/p/ebc46dc9c2ee
        text = fil.sub('', text)
        text = re.sub('\n+', '\n', text)
        text = re.sub('[ \n]+$', '', text)
        listb.insert(END, '共{}字，开始处理...'.format(len(text)))
        listb.see(END)
        self.text = text
        self.mark(parser)
        self.vocalize()
    ...
    def to_vsqx(self, addr='', start=True):  # phonemes to vsqx
        ...
        pre = 5000
        total_time = self.total_time - 4900 + pre
        f0_level = 63
        pbs = 13
        ...
        for note in note_list[:]:
            ...
            for pit in note[3]:
                ...
                if p < -8190:
                    p = -8190
                text_insert += '\r\t\t\t<cc><t>{}</t><v id="P">{}</v></cc>'.format(round(pit[0] + pre)-5000, round(p))
            for intns in note[4]:
                if 'ang' in note[0]:
                    intns = (intns[0], intns[1] * 0.6)
                text_insert += '\r\t\t\t<cc><t>{}</t><v id="D">{}</v></cc>'.format(round(intns[0] + pre)-5000,
                                                                                   round(intns[1] * 64))
            if note[0][0] == 'sil':  # 调整间隔note，但参数不取消
                note_list.remove(note)
            percent = round(counter/total, 2)
            if percent in to_print:
                listb.insert(END, '关键循环完成度：'+ str    (percent*100)+'%')
                listb.see(END)
...
\t\t\t\t\t<v id="vibType">0</v>
\t\t\t\t</nStyle>
\t\t\t</note>""".format(start=note[1] + pre - 5000, dur=note[2], f0=f0_level, py=py, ph=dict_ph+er_ph+nasal_ph, vel=vel)
...
\t</aux>
        ...
        text = text_head + text_length + text_mid + text_pbs + text_insert + text_last

        global outnumber
        file = open("./outputs/output" + str(outnumber) + '.vsqx', "w", encoding="utf-8")
        file.write(text)
        file.close()
        if start:
            os.system('start outputs/output' + str(outnumber) + '.vsqx')
            sleep(0.5)
...
#获取当前时间戳，保留整数部分
time_log = str(round(time()))
#设置日志格式
LOG_FORMAT = "%(asctime)s / %(levelname)s / %(message)s"
DATE_FORMAT = "%Y/%m/%d %H:%M:%S"
try:
    logname = 'logs/log_'+time_log+'.log'
...
#设置获取版本号的网址
version_url = 'https://s2s.rth7.com/version'
#设置请求头
headers = {
...
def check():
    #获取最新版本号
    try:
        getversion = requests.get(version_url,headers = headers)
    except:
        logging.warning('get version error#1')
        messagebox.showwarning(title='错误', message='在检查更新时出现错误。\n这可能是因为网络未连接，或者其他错误。\n你可以手动前往'+download_url+'下载最新版。\nERROR #1')
    else:
        if getversion.status_code == 200:
            getversion = getversion.text
            #比较版本号
            if getversion != version:
                logging.info('check updates, yes')
                #弹出对话框
                check = tkinter.messagebox.askyesno(title = '发现新版本', message = '发现新版本（'+str(getversion)+'），是否跳转到下载页面？')
                if check:
                    webbrowser.open(download_url, new = 0, autoraise = True)
                    exit()
            else:
                logging.info('check updates, no')
...
class Danmu():
    ...
    def get_danmu(self):
        ...
        try:
            ...
            logging.error('get danmu error#2')
            messagebox.showwarning(title='错误', message='在获取弹幕时出现错误，这可能会导致弹幕无法正常获取。\n这可能是因为网络未连接，或者其他错误。\n请检查网络，或确认是否能访问live.bilibili.com。\nERROR #2')
        #解析弹幕列表
        for content in html['data']['room']:
            ...
            time_danmu = mktime(strptime(timeline, '%Y-%m-%d %H:%M:%S'))
            #比较时间
            if time_danmu > time_now:
                time_now = time_danmu
                #记录发言
                msg = nickname + '说' + text
                logmsg = 'danmu' + ':' + timeline + ' ' + nickname + ':' + text
                #小写字母转大写字母转中文
                msg = msg.translate(Sletter2Bletter)
                msg = msg.translate(Bletter2hans)
                #打印消息
                listb.insert(END, logmsg)
                listb.see(END)
                #朗读
                readmsg(msg)
                #保存日志
...
def about():
    tkinter.messagebox.showinfo(title = '关于', message = '''SingToSpeak(S2S) by Xwei
版本：v'''+version)
...
def log():
    system(r'notepad logs/log_'+time_log+'.log')
    
#菜单栏
menubar = tkinter.Menu(window)
#Open放在菜单栏中，就是装入容器
menubar.add_command(label = '检查更新', command = check)
menubar.add_command(label = '关于', command = about)
menubar.add_command(label = '帮助', command = help)
menubar.add_command(label = '日志', command = log)
...
#创建标签
tkinter.Label(frame_t, text='房间号：', width=10, font=('Arial', 12)).pack(side=tkinter.LEFT)
#显示成明文形式
default_text = tkinter.StringVar()
default_text.set("25537837")
...
def start_point():
    try:
        ...
        logging.warning('live number error#3')
        messagebox.showwarning(title='错误', message='输入的房间号格式不正确，请输入正确的房间号。\n房间号可以在房间页面获取，如25537837。\nERROR #3')
